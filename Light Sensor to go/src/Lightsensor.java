...
public class Lightsensor {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			errorIntegrated = (1 - DELTA_T * 1) * errorIntegrated + DELTA_T
					* error;
			errorDerivated = (error - lastError) / DELTA_T;

			System.out.println(errorIntegrated);

			float motorBreak = 0; // Math.abs(errorIntegrated) / BREAK_COOEFICIENT * BASE_SPEED;
			if (Math.abs(errorIntegrated) > BREAK_COOEFICIENT) {
